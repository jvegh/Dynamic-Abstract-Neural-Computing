...
\section[Modeling]{Modeling\label{sec:SingleModeling}}


\quotationbox{
"What I cannot create, I do not understand"~R.~P.~Feynman}

\quotationbox{"There is a widely accepted distinction between merely modeling a mechanism's behavior and explaining it. \dots Some models are data summaries. Some
...
\subsection{Approaches\label{sec:Single-ModelingApproaches}}
The \hyperlink{spatiotemporal}{spatiotemporal} neuronal operation is too complex and fast to enable for providing a
sufficiently detailed and accurate observation. 
...
'foreign' (external), time-dependent current into the cell, is not in their equations. Fourth, the invasion (fixing
one of the interdependent parameters) leads to wrong conclusions, such as 'demonstrating' otherwise non-existent $K^+$ current, discussed in section~\ref{sec:Fallacies-OutwardK}.
...
\subsection{Types\label{sec:SINGLE_ModelTypes}}

Our model is surely a physical model.
...
\subsection[Disciplinarity]{Disciplinarity\label{sec:SINGLE_ModelDisciplinarity}}
...
\section{Creating the brain\label{sec:SINGLE_ABSTRACT_CREATING}}
