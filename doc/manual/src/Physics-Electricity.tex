...
\physicspanel{phys:ElectricDichotomies}{Electricity-related dichotomies}
{
One of the famous dichotomies (see section~\ref{sec:Physics-Dichotomies}) is that there exist laws, separately,
...
between them. In biological cells, the usual "infinite" conditions (infinitely far, infinitely small, infinitely fast, etc.)
of the classical electronics are not fullfilled: it depends on the context what can be omitted with respect to some other quantity.
...
but it is explicitly considered only in the slightly non-ordinary 
...
can be interpreted in unchanged form, they must be reinterpreted. The movement of charges is interpreted as current (and, simultanously, as mass transport),
but the are moved not necessarily only by an electric field;
furthermore,
...
\textit{The real complication and difference is, however, that the interactions have different speeds, which situation rarely happens in physics.}
...
through forces and an infinitely small test charge. In living matter,
neither is valid in an unchanged form: constrains and physical/chemical additional forces are present, furthermore, the charges of the carriers are not
negligible compared to the charges that generate the field; furthermore, they are quantized. \textit{Since the carriers have mass and a different propagation mechanism, 
one cannot apply laws of disciplinary electricity, valid for "fast currents", since they are valid for "slow" currents in a different form}. One may have fundamental problems with the mathematical descriptions: 
the discrete nature of charge carriers question marks  the differentiability of the corresponding functions
\hyperlink{math:Differentiability}{see the math panel \ref{math:Differentiability}},  
the charge-mass dichotomy of ions, discussed in section~\ref{sec:Physics-Dichotomies}, the interpretation 
of scientific disciplines, the spatiotemporal features,
discussed in \hyperlink{phys:Spatiotemporal}{Physics panel \ref{phys:Spatiotemporal}},
the simultaneity of different phenomena.
...
although the transition between the discrete and continuous views
...
In a useful first construct, the charge carriers are hard-sphere objects.
The most important opening idea, electrically, is that they have a property called "charge" which is the same size, but opposite in polarity for the ion and the electron; furthermore, the ions can have multiple charge.
The ions are about $50,000-100,000$ times heavier and about the same factor larger than electrons, and this difference leads to crucial other differences, for example in their speed, conduction mechanism and thermodynamic features.
For the sake of simplicity, we always think about positive ions,
although we know there are also negative ions.
An essential implication is that an ion and an electron will strongly attract each other, while two positive ions or two electrons would strongly repel each other. 
In closed biological space, the repulsion combined with the corresponding counterforces may lead to 
mechanical pressure.
...
\physicspanel{phys:LocalChargeConservation}{Local charge conservation}
{
...
In biology, especially in connection with the operation
of neurotransmitters, it is not always clear what happens with the "created" and "disappearing" charges.
...
the charge moves from one place to another with a finite speed. The case is similar to the case of general relativity, and also its reason is the same: the finite speed of interaction. In the general theory of relativity, local and global energies had to be introduced; for its mathematical handling, see, also \href{https://en.wikipedia.org/wiki/Noether's_theorem}{Noether's theorem}.
...
behave as electrons in metallic conductors. Although biology does not say so explicitly, implicitly it says so by using the equations
describing electrons in solids.} Using the fundamental laws of electricity without changes means assuming that the ions
travel at the same speed in electrolytes as electrons in metal conductors, surely misleading researchers (by introducing 'delayed current', 'delayed ion channels', and similar obscure notions).
\index{delayed current}
\index{ion current!delayed}
One must be especially careful that the \hyperlink{Biologicalcircuits}{electric testing circuits} are hybrid: 
...
A charge generates an \href{http://hyperphysics.phy-astr.gsu.edu/hbase/hframe.html}{electric field}. 
In the continuous view,
...
Given that the electric field integrated over a closed surface is proportional to the charge enclosed in the volume, furthermore the pressure on the surface is proportional to the
number of ions enclosed in the volume; it should not be 
surprising that pressure and the electric field are proportional to each other. Changing the number of ions
in a closed volume changes the related voltage and
pressure at the same time.

\physicspanel{phys:ElectricalMonopole}{Electrical monopole}
{
...
In connection with \href{http://hyperphysics.phy-astr.gsu.edu/hbase/hframe.html}{voltage},
...
the contribution of a \hyperlink{ThermodynamicElectricField}{pseudo-electric field} arising from thermodynamics, see 
section~\ref{sec:Physics-ElectrolyteThermalField}.
...
to the membrane}}, as we discuss in section~\ref{sec:PHYSICS_ConductionMechanism}.


It is important, that a voltage can be generated either by
driving a current through a solid (in the continuous view) or placing charges in different points, typically on the surface of some biological object (in the discrete view). Electricity explains that voltage can be measured without or with electric current; having entirely different physical background.
Accumulating charge on an insulator, such as a membrane's surface, results in producing an electrostatic field. Of course, while charging, a current flows and energy is needed to produce a voltage, but it is a one-time action; the created potential difference persists.
...
\index{Stokes-Einstein speed}
Notice the difference the thermodynamic feature of charge carriers
means: only an electric field can move the electrons, while a
...
a current. Hovever, it is a kind of potential energy.
 Instead, its gradient ($E=U/d$) has such an effect, where $d$ is the distance. The field exerts force on a charge carrier in the respective
...
until charge carriers appear in the axon. It is important to distinguish potential and the difference of potentials in two different points.
The latter, together with the distance of the points, leads to a gradient.
...
in volumes and surfaces when charge relocates (for any reason) due to different processes. It differs from the case of physical electricity
where the instant interaction means that the changes
reach the different parts of the volume or the surface instantly. 
In the case of living matter, internal processes change
continuously the mass and charge distribution: no uniform
bulk concentration and uniform membrane potential exists.
...
Physics interprets current that charge carriers traverse
between to distant spatial points, this way naturally
implying a charge transmission (conduction) mechanism.
\index{charge transmission}
\index{charge!conduction mechanism}
...
a cross section $A$. 
Technical electricity handles current as an infinitely thin 
electrical fluid.
At any point and at any time, the incoming charge equals the outgoing charge.
\index{Kirchoff's Laws}
 Kirchoff's law expresses \textit{charge conservation at any point, in a differential form}.
The correct definition of \hypertarget{electric_current}{current} is a differential one: $i=dq/dt$, as given for physiology by Appendix A.3.4 in~\cite{JohnstonWuNeurophysiology:1995}.
By using this definition, A.3.5 in~\cite{JohnstonWuNeurophysiology:1995} correctly defines that "Ohm's law states that the ratio of voltage to current is a constant:
...
Recall that the definition of the current is given
at a cross section (i.e., an infinitely thin volume element),
and it is valid along the current's path only if the current
is the same across the volume. If the speed of the
charge $dq$ is finite, a temporal difference belongs
to the entrance and exit points of the charge.

\physicspanel{phys:DisappearingCurrent}
{Disappearing current}
{
The other way round, as given by Eq.~(1.4) in~\cite{KochBiophysics:1999}, is wrong.
"It is straightforward to describe the dynamics of this circuit by applying Kirchoff's
...
and only in the case of 'instant interaction' (the corresponding speeds differ by six orders of magnitude),
which is not the case for ions in electrolytes.
\index{Kirchoff's Laws}
\textit{The charge, instead of current, cannot disappear (and cannot newly appear either), it has to go somewhere}. \textit{There is no conservation law for the time derivative of the charge, only for the charge itself.
...
%
\textit{The current can be interpreted in an integral form as current conservation only if the electric interaction is instant,
the discrete components are point-like,
and the measured system is closed} (or complete).
The integral form  is an approximation, (more or less) valid for classical physics,
but surely not valid for biology. 
The charges can temporarily disappear (they can be stored, delayed, say on the membrane of axon or exit to a non-measured
...
The situation is apparently similar to the cases of charges in inductive, capacitive, etc. currents in technical electricity. The significant difference is that in the technical electricity the sum of the charges in the different currents
is constant, in biology it is not known where are the charges
in a delayed current.
}


\subsubsection[Conduction mechanism]{Conduction mechanism\label{sec:PHYSICS_ConductionMechanism}}
...
classical electricity and biology considers charges and their moving.
...
\else
\includegraphics[width=.30\textwidth]{fig/SolidResistance.png} & 
\includegraphics[width=.30\textwidth]{fig/IonicResistance.jpg} \\
\fi
\hline
...
The process is mostly reversible.  See also section~\ref{sec:Physics-HeatAbsorption} and Equ.(\ref{eq:StokesEinsteinSpeeddV}). 
Fig.~\ref{tab:Electric_ResistanceTypes} shows that resistance 
...
In biological matter, the volume is more or less limited (is finite), there may be charged objects present that make the propagation non-isotropic. Furthermore, the charge carriers are non-uniform
that introduces thermodynamic interaction in addition to the electrical interaction.
Ions, under the electric/thermodynamic field (and maybe mechanical constraints) accelerate to the \hyperlink{phys:StokesSpeed}{Stokes-Einstein speed} (see Eq.(\ref{eq:StokesSpeed})), move against a friction in the electrolyte.
\index{StokesEinstein speed}
...
\paragraph{Conduction under constrains\label{sec:PHYSICS_ConductionConstrained}}

If the acceleration occurs in a narrow tube 
such as an ion channel (the ion channel has a typical size of a few nanometer, the mean free path is of size ten times less), the wall prevents gaining momentum in the 
direction perpendicular to the direction of acceleration. 
Given that the collisions are elastic, (almost) no energy dissipates;
so, in principle, the process is reversible.
Strangely enough, this way the electrical force
accelerates also the neutral molecules; furthermore, their momentum and energy (using an elastic membrane) can fully contribute to the operation
of neuron (forms the \gls{AP}). 


This way, in a narrow tube (such as an ion channel) the momentum exchange
...
Vice versa, if the bulk of the particles are moved mechanically (say, when an ultrasound wave
 presses the neuron or a rush-in process transfers elastic energy to the membrane), the one-directionally moving
...
channel (and so, can calculate resistance), but the behavior of the resistance/conductance is 
...
The \gls{AP} initially produces an impulse force that compresses the membrane. The compressed membrane, while performing a damped oscillation, pushes and pulls the incompressible electrolyte that contains a small amount of dissociated ions, this way (per definitionem)
...
\indexit{Stokes-Einstein speed}, the voltage-, current-, temperature and pressure waves are synchronized, as observed. The different 
experimental observations measure the same effect
by using their disciplinary measuring devices as different quantities.
The energy of the ions cannot dissipate; instead, the neutral and charged 
...
This way, by introducing the correct interpretation of "resistance" for electrolytes, the seven-decades-old
...
Voltage and current, the secondary electric quantities, derive directly from charge, the primary quantity, and (except time) no external quantity is used. In contrast, when deriving resistance or conductance, one measures how the tested object relates to electricity. At this point comes to light the meaning of E.~Schrödinger's warning about how do we tests in
physics laboratories. Although physics knows (based on their 
electric behavior) conductors, insulators and semiconductors, the instant interaction is tacitly included and
...
without any cause), that changes the rules of the game in 
...
and deviate from the Ohmic case.
However, energy and the charge conserves (instead of current as~\cite{KochBiophysics:1999}, page 9, claimed), so we could introduce 
...
by Stokes's Law). There are, however, further crucial differences between the current propagation's mechanisms in solids and in biological materials. 
...
and the permanently present electric field may change the phenomena drastically: the current may appear with a delay with respect
to what is expected having "fast" charge carriers in mind (and their behavior in the laws). 
Furthermore, biologically active structures may "produce"
charge carriers inside.
As a consequence, when deriving ternary quantities from 
those secondary quantities, we may use non-matching quantity pairs (measured at different times), that produces wrong (unphysical) conclusions.
...
\physicspanel{phys:MeasuringConductivity}{Measuring conductivity}
{
\textit{When measuring resistance by dividing the measured values of voltage and current,
a quantity of dimension resistance or conductance can surely be derived.}
...
Of course, even that instrument (see Fig.~\ref{fig:Physics-Multimeter}) cannot measure directly the ratio of
...
and displays the calculated result, using Ohm's Law, in units of resistance/conductance.
The device uses the differential form of 
deriving the current: measures 
the voltage and the current at the 
same time. Here comes into play the cheating: instead of actually measuring the voltage, the device uses voltage measured earlier and current value measured at the time of measurement.
In the case of 'instant interaction', the two measurements are simultaneous.
...
\physicspanel{phys:OtherMeasuringEffects}{Other Measuring Effects}
{
In addition to the effect described in \hyperlink{phys:MeasuringConductivity}{physics panel~\ref{phys:MeasuringConductivity}}, two more kinds of issues come to light when measuring in biological circuits. As we mentioned, although the measurement device
...
Forgetting this fundamental feature results in more or less
...
experimenter wants to measure. It cannot separate its own current that its required voltage produces
from the current that the tested object autonomously generates.
It works with 'instant' electricity, 
...
the voltage change gets observable. In the case of changing gradients, \textit{the conductance meter uses non-matching pairs of voltage and current data}: the device is designed for measuring in a 'steady-state'.
...
value pair. Apparently, the conductance increases as the 'foreign' current (that is compensated by the feedback) increases,
...
the  dynamic component is practically missing: the charge instantly arrives 
...
\physicspanel{phys:StokesSpeed}{Stokes speed of ions}
{
...
\physicspanel{phys:StokesDiffusionCoefficient}
...
The \gls{AP} arrives at the beginning of the axon in the form of a traveling wave of a  slow current (an ion packet delivering ions). Recall that ions move in the "skin"
