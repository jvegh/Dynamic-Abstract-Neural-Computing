...
class scGenComp_PU_Bio : public scGenComp_PU_Abstract
{
  public:
    ...
    void Membrane_R_Set(double R = 100.)    //< in MOhm
    {   m_NeuronData->m_Membrane_R = R*THOUSAND; Membrane_Tau_Set(R,m_NeuronData->m_Membrane_C);}    // Store the value in kOhm
    void Membrane_C_Set(double C = 100.)    //< in pF
    {   m_NeuronData->m_Membrane_C = C*THOUSAND; Membrane_Tau_Set(m_NeuronData->m_Membrane_R*THOUSAND,C);}    // Store the value in fF
    double Membrane_Tau_Get(void)   //< Return $\tau$ in ms
    {   return m_NeuronData->m_Membrane_Tau /THOUSAND;}
    double MembraneResistance_Get()
    {return m_NeuronData->m_Membrane_R*exp(m_NeuronData->m_ResistanceResolutionExponent);}

protected:
    //! Puts the PU to its default state
    virtual void Initialize_Do();
    void OutputItem();
    void Membrane_Tau_Set(double R, double C)    //< Tau in usec
    {   m_NeuronData->m_Membrane_Tau = R*C /THOUSAND /THOUSAND; } // in usec = 10 ms
