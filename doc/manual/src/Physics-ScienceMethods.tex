% Concepts of science, for DANCES
...
In his very accurately formulated question, 
...
It based its laws on abstractions/idealizations that were concluded by approximations and contained \textit{only one} of these concepts. 
Inventing non-independence of space and time led to creating the theory of relativity.
Under certain circumstances one must treat particles as a kind of continuous wave, and continuous waves as a particle.
All those inventions had far-reaching consequences.
...
We understood that the Newtonian approach has its limitations; finite-velocity interactions can only be described by using a different kind of mathematics: by introducing the concept of space-time.
...
science's different disciplines, and even more for biology and physics.}
...
In most cases, the approach can be --more or less-- successful. Notice, however, that in the case of a jump
we neglect the need to explain why and how the transition happens.
For the slower, well-observable processes,
...
\quotationbox{
 “\textit{In thinking about the physical basis of the action potential perhaps the most important thing to do at the present moment is to consider whether there are any \hypertarget{UnexplainedObservations}{unexplained observations} which have been neglected in an attempt to make the experiments fit into a tidy pattern}”~\cite{HodgkinConduction:1964}, page 70.
 We add that the disciplinarity of science naturally
 leads to unobserved quantities: the quantities missing from 
 a discipline remains out of sight of the theory and the interpretation.
}
...
It happens despite Hodgkin's opinion about the %\hyperlink{UnexplainedObservations}
...
\subsubsection[Disciplinarity]{Science disciplinarity\label{sec:Physics-ScienceDisciplinarity}}
...
Newton's laws deal with masses only. Coulomb's laws deal with charges only.
We use the abstraction "charge"
...
The charge has no mass, but the charge carrier does.
We must not forget, however, that \textit{those laws have been
derived based on approximations and omissions}, and so they also \textit{have
...
we must scrutinize whether all laws we use are applied within their range of validity. 
 "Biological laws are much more difficult to find than physical ones"~\cite{VollmerLimitsOfBiology:1995}.
We add that mainly because those laws cannot be uniquely mapped
to the disciplines of physics. The charge and mass can be separated conceptually, but not physically.
...
Connecting two abstract notions generally results in counter-intuitive experiences.
Interestingly, similarly to relativity, their velocity establishes the connection between the two quantities here.
...
However, we base the correspondence on geometrical considerations and, due to the unseparable connection between the two properties, we cannot apply statistical laws based on one property in an unchanged form.
...
In some cases, closed spaces give rise to counterforces that fundamentally change the processes that take place.
...
Describing living matter is undoubtedly more complicated than describing inanimate nature and requires a revision of several approximations. However, it is possible.
...
that
can be applied without thinking about whether they can be applied 
to the conditions of living matter.}
...
\quotationbox{
%\begin{advanced}
"How can \textit{the events in space and time} which take place \textit{within the spatial boundary} of a living organism be accounted for by physics and chemistry?"~\cite{Schrodinger:1992}
}
...
so the transport itself gradually changes the gradients (the field itself). 
The transferred ions decrease the field in the volume they departed from
and increase it in the volume where they arrived; furthermore, the concept of 'field' is slightly different from the classical one, see
below.
% section~\ref{sec:Physics-Fields}. 
We must consider that the resources are finite. A main source of confusion is that the phenomena happen in a limited and closed region of space (\textit{within the spatial boundary}), and we study processes (in a period instead of a moment) where the environment is not "infinitely far" from the studied object (\textit{the studied process interacts with its environment}), and
the forces exerted on it belong to different disciplines.
The boundaries of a closed volume
...
The more distant parts of the biological cell will see any change in the local values of the state variables at different times (with a delay). 
...
Co-existing processes may affect each other by \hypertarget{ChangingResources}{changing the available resources}:
...
cannot surely be applied in unchanged form to living objects.  

\subsubsection[Fields]{Fields in science\label{sec:Physics-Fields}}

The concept of force fields is fundamental for science
since I.~Newton introduced that remote objects can interact with each other. Traditionally, a field is defined by the force a test charge (or mass) would experience at a specific point in space, even if no charge/mass is currently there (assuming an infinitely small charge or mass, despite that we know their discrete nature).
\index{force field}
R.~P.~Feynman questioned whether fields were fundamental physical entities or simply convenient mathematical descriptions of the "condition of space".
He cautioned against "mistaking the map for the territory," suggesting that while (electric and magnetic) fields are useful, they might not be the ultimate reality.
Anyhow, the fields are created by nearby ions which 
can change their positions during the interaction,
furthermore, the ions under test interact with them
and this way change their environment.
Although the fundamental interactions forces and laws
are the same that we are used to when testing objects
from the inanimate nature,
the "construction" is entirely different.
This difference needs much attention because 
the laws concluded for objects of inanimate nature
are not valid for objects in living nature in unchanged form.

Mass and charge may have the corresponding fields 
given that they have well-known remote interactions. 
However, thermodynamics does not consider either
gravitational or electrical fields: the interaction 
is implemented by short-range interaction (direct collisions). Although there are
theories for both fields where (typically virtual) particles
represent the action of the field, those particles
are not identical with the sources of that field.
To extend the concept of field to thermodynamics,
one must extend the idea of deterministic forces
with stochastic forces; so the resulting 'field'
is of stochastic nature.
\index{stochastic force}
\index{force!stochastic}
In the case of ions in electrolytes (particularly in
structured biological matter), they move under the effect of
electric and thermodynamic fields.
In their laws of motion, one has to add electrical and thermodynamic fields, i.e., a deterministic and a stochastic field. The result is stochastic, anyhow.
The case is further complicated by the fact that 
the propagation speeds are different for the two cases.
One can tell only stochastically valid statements
and must derive special laws of motion
\index{laws of motion}
(say, for the selectivity of ion channels; see section~\ref{sec:Physics-IonSelectivity}).
...
Paper~\cite{ThermodynamicAPDrukarch:2022}, which, in the context of this section, tends to be the precursor of our ideas, sheds light on some remarkable dichotomies,
...
will be used for biology which is not one of the disciplines of inanimate science.
 In biology, that set does not necessarily 
...
As discussed in connection with the Onsager's relations, a disciplinary (incomplete) measurement does not discover that an unexpected change (an unexplainable "miracle") in the value of one of the quantities belonging to another discipline is accompanied by a change in the
...
of mechanical/thermodynamic quantities which were not measured, partly due to the obvious 
...
The disciplinary separation in classical science does not apply to the study of living matter.
...
so one can use the equations assuming 'instant interaction' (aka an infinitely
large interaction speed, and, consequently, an infinitely
large acceleration),
which leads to non-physical explanations of the physiological observations (such as 'delayed current').
...
\physicspanel{phys:ElectricalDichotomy}{Electrical Dichotomy}
{

\textit{To deliver a
current, one needs moving charged particles that need exerting of some
external (electrical, magnetic, or chemical) force or a mixture of forces.
We have speeds of 
...
\physicspanel{phys:Atomicity}{Atomicity}
{
...
Connecting those views was one of the tasks performed by thermodynamics for particles with no long-range interactions. Inventing discreteness of energy led to quantum mechanics.
...
However, there is no similar discipline for electricity,
...
A reversible process can proceed in either direction in time, see \hyperlink{phys:Reversibility}{Physics panel \ref{phys:Reversibility} on reversibility}.
...
\physicspanel{phys:Reversibility}{Notes on reversibility}
{
...
\physicspanel{phys:Speed}{Notes on speed}
{
...
speeds and their combinations. More discussion follows in section~\ref{sec:Physics-ThermodynamicsMixingSpeeds}.
...
that \textit{also thermodynamic forces can move the ions},
given that they have inseparable mass.


\physicspanel{phys:LowSpeed}{Notes on low speed}
{
...
In that time, E. Schrödinger introduced his famous Law Of Motion into quantum-mechanics entirely analogously
...
\physicspanel{phys:SpeedDependence}{Notes on speed dependence}
{
...
basically distinguished by considering their \textit{speed dependence} that actually means \textit{explicite time dependence}.
...
The Telegrapher's equations (unfortunately, also used to describe biological signal transfer)
...
\physicspanel{phys:SpeedOfLigh}{Speed of light}
{
...
is finite, but he measured its value with a sufficient accuracy. Rømer
...
\physicspanel{phys:SpeedInNeuroscience}{Speed in neuroscience}
{
...
a single equipotential surface (in other words, classical physics' instant
...
(forgetting that Ohm's Law is valid only for the classical physics's 'instant
...
represent a "slow current" (compared to the "fast current" represented by electrons and their charge transmission method), biology introduced changing conductance, delayed current, rectifying current, and so on; effectively blocking
the understanding of the neural operation.
}


\physicspanel{phys:FiniteSpeedInteractions}{Finite-speed interactions}
{
...
\physicspanel{phys:ProcessesVsStates}{Processes vs states}
{
\textit{Life, including the brain's operation, is dynamic.} 
The finite speed introduces something unusual into science:
processes  (transitions), rather than jumps between states must be considered.
The processes connect states and we need the corresponding laws of motion to connect them. 
...
\physicspanel{phys:SpeedInLawsOfScience}{Speed in laws of science}
{
...
range.
\textit{We must not describe the axon or the membrane with the non-differential form of the Kirchoff equation:
...
\textit{Studying electrical phenomena on structured
...
\physicspanel{phys:Spatiotemporal}{Spatiotemporal}
{
...
\mathematicspanel{math:MathematicsLanguage}{Mathematics as a language}
{
...
\mathematicspanel{math:Differentiability}{Differentiability}
{
...
in technical electricity, surely they are: for a large number of
...
The definition of a \hypertarget{PartialDerivative}{partial derivative} of a function of several variables is its derivative with respect to one of those variables,
...
\mathematicspanel{phys:Statistical}{Statistical}
{
...
\index{non-ordinary law}
some non-ordinary laws are to be derived to describe life.

Classical physics is simplifying the things until it can attribute 
...
the attibutes of the substances. As E.~Schrödinger has drawn attention to, "the construction [of living matter] is different from anything we have yet tested in the physical laboratory".
...
tested sample~s size) has led to establishing new scientific fields,
...
We discuss some of the relevant terms and notions of physics,
...
Schrödinger wanted to emphasize the opposite: there is no new force (no unknown new interaction), only studying living matter needs different testing methods in the physical laboratory. He suggested to answer the question
...
\physicspanel{phys:OnModels}{Notes on physical models}
{
The physical models consider infinitely large volumes,
surfaces, distances; furthermore, and most importantly: instant interactions (aka infinitely large speeds). Is the cell large enough to
...
We also show in the section~\ref{sec:Physics-Instant} about the 
...
\physicspanel{phys:WrongOmissions}{Notes on wrong omissions}
{
...
it is one of the hardest tasks to establish relations between
