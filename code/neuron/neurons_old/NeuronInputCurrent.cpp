 ...
 *  @brief  The phsics-based abstract neuron's input current
 */
/*
...
NeuronInputCurrent::
    ...
    ,m_Parameters(Parameters)    // and the type of current
    ,m_CurrentType(CurrentType)        // Beginning of the time scale
    ...
    Value_Get(double TT)
{
    ...
    case NeuronInputCurrent_t::nict_RushIn:
    case NeuronInputCurrent_t::nict_PSP:
        return m_Parameters[0]*(1-exp(-m_Parameters[1]*T))*exp(-m_Parameters[2]*T);
        break;
    ...
    Gradient_Get(double TT)
{
    ...
    case NeuronInputCurrent_t::nict_PSP:
        return m_Parameters[0]*(
                      m_Parameters[1]*exp(-m_Parameters[1]*T-m_Parameters[2]*T)
                      -m_Parameters[2]*exp(-m_Parameters[2]*T)*exp(1-exp(-m_Parameters[1]*T))
                      );
        break;
        // No need to check current's type
    default: assert(0);
