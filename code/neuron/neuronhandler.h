...
/**
 * @brief The NeuronHandler class
 * Manages construction and destruction of a neuronal design, when
 * selecting between neurons. Manages all interaction and control of the
 * current processor.
 */
class NeuronHandler : public QObject {
  Q_OBJECT

public:
  /// Returns a pointer to the NeuronHandler singleton.
  static NeuronHandler *get() {
    static auto *handler = new NeuronHandler;
    return handler;
  }

  /// Returns a non-const pointer to the currently instantiated processor.
  static NeuronProcessor *getProcessorNonConst() {
    return get()->_getProcessor();
  }

  /// Returns a pointer to the currently instantiated processor.
  static const NeuronProcessor *getProcessor() { return get()->_getProcessor(); }
#if 0
  ...
  static const NeuronID &getID() { return get()->_getID(); }
#if 0
...
private:
#if 0
  /// Private implementations of the NeuronHandler singleton functions. For
  /// documentation, refer to their static counterparts above.

  void _loadProgram(const std::shared_ptr<Program> &p);
#endif
  NeuronProcessor *_getProcessor() { return m_currentProcessor.get(); }
  const NeuronProcessor *_getProcessor() const {
...
#endif //0
  const NeuronID &_getID() const { return m_currentID; }
#if 0
  ...
  NeuronID m_currentID;
  #if 0
  RegisterInitialization m_currentRegInits;
#endif //
  std::unique_ptr<NeuronProcessor> m_currentProcessor;
#if 0
   ...
   */
  QTimer m_neuronStateChangeTimer;
