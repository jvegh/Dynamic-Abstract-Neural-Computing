...
\section{Science issues\label{sec:Physics_ScienceIssues}\label{sec:Physics-Abstraction}}
...
\subsubsection{Schrödinger's points\label{Physics-Schrödinger}}
...
in their actual environment, and in 'working state'. As we did with non-living matter, we must develop and gradually refine the testing methods as well as the hypotheses.
Moreover, we must not forget that our methods refer to 'states', and this time, we test 'processes'. Not only in measuring them but also in handling them computationally, we need nore or less different algorithms.
\end{itemize}

\index{Schrödinger, Erwin}

\subsubsection{Processes vs states\label{Physics-Processes}}
...
(a problem of finite resources).  We must consider that the resources are finite.
...
Connected the features of the microscopic discretete constituent objects  
...
We have understood that the Newtonian approach has its limitations; finite-velocity interactions can only be described with a different kind of mathematics: by introducing the concept of space-time.
...
speed (the Einsteinian model) or connects the discrete and continuous views.
However, \textit{the finite interaction speed
...
in the case of biological object and processes.
...
Science has derived laws for the forces acting on those abstract objects, such as Newton's universal law of gravitation and Coulomb's law of electricity. Then one could apply Newton's laws of motion. Experience shows that the generated forces, regardless of their origin, can be summed, and one can apply the laws of motion using the resultant force (see below for a discussion of the resultant force and its application in biology). Howevr, notice that those experiences are based on the Newtonian
...
Connecting two abstract notions always results in counter-intuitive experiences.
...
%\subsubsection{Speeds\label{sec:Physics-Speeds}}
...
$dQ$ cannot be smaller than the elementary charge. Mathematically, one can say that the time course of a single
...
Do those statistical laws valid for the quasi-continuous 
infinite volumes remain valid for non-infinite closed volumes
or for the individual ions? (recall that thermodynamics 
follows the hybrid path that calculates the \textit{probable number}
