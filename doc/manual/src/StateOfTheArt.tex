...
Dangerous is the tradition of applying ad hoc mathematical formulas
...
describing electrical circuits, it assumes that electrons constitute 
the currents.
...
Hodgkin and Huxley in 1952~\cite{HodgkinHuxley:1952} enormously advanced neurophysiology by providing a series of \textit{observations}, using a mathematical description, on neuronal function.                                                                                                 However, as they warned, many of the \textit{mechanisms} they hypothesized must be fixed or replaced: "must emphasize
...
Editors of neuroscience-related journals disagree with Feynman. They do not want a "new understanding".
...
We only quietly note that the model by Aristotle and Ptolemy
has been extensively studied for over 14 centuries until Newton rethought the science, then for two centuries Newton's model has been extensively studied until Einstein rethought scientific principles. Maybe the Hodgkin-Huxley model should not be an absolute truth.
}
...
Those parts, that need a different background or can be
skipped at first reading, are organized into panels
...
{
Physics panels contain the strict physical background; a material
which wan be difficult without the required background.
}

\mathematicspanel{intro:MyMathematicsPanel}{An example mathematics panel}
{Mathematics panels typically contain equations and 
their explanations; a material
which wan be difficult without the required background.
}
...
Those materials are put into panels, such as
\hyperlink{intro:MyPhysicsPanel}
{Example physics panel~\ref{intro:MyPhysicsPanel}},
\hyperlink{intro:MyMathematicsPanel}
{Example mathematics panel~\ref{intro:MyMathematicsPanel}}, or
\hyperlink{intro:MyNumericsPanel}
{Example numerics panel~\ref{intro:MyMathematicsPanel}}

                                                                                                       
We proceed in a \hyperlink{zigzagdiscussion}{zigzag way}
when discussing different levels of understanding.
In chapter~\ref{ch:Single} we introduce the abstract
term and notions that  allow uus to discuss neuronal operation withhout being lost in the physiological and 
biochemical details.


Our discussion is closely related to %\hyperlink{Physics}
{physics}, discussed in chapter~\ref{ch:Physics}
where we establish the needed science
concepts in an explicite form from the first principles of science.
...
and reinterpret some concepts of physiology in light of the correctly formulated notions.
We keep the discussion on functional level,
going into structural and operational questions only
...
After that we discuss the wrong conclusions (due to using the laws on inanimate science in unchanged form) leading to commonly used fallaciesin chapter~\ref{ch:Fallacies}.
