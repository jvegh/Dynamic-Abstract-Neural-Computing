...
//    do
  //  {
        ...
        sc_core::sc_time ThisTime = sc_core::sc_time_to_pending_activity(); // Make a single simulation step
            uint64_t DiffTime = ThisTime.to_seconds()*1000*1000;
        m_CurrentTime = sc_core::sc_time_stamp();
...
//    }   while(!ThisTime.to_seconds()*1000*1000); // Do not send event at zero time difference
        emit eventHappened();
