...
\physicspanel{phys:CellForces}{Comparing forces exerted on ions in cells}
{
One can compare the force due to \textbf{\hyperlink{phys:CellPressure}{Physics panel~\ref{phys:CellPressure}}}
...
\physicspanel{phys:RushinSpeed}{Estimating the rush-in speed}
{
	...
	the ions' speed (Fig.~3 in~\cite{HodgkinHuxley:1952}, quantitatively underpins the hypothesis, see~\cite{VeghStokesEinstein:2026}) both in axons and ion channels. If so,
	...
	$50~\mu m$ \gls{AIS} generates a $2*10^3~\frac{V}{m}$ electrical gradient, and
	\cite{HodgkinHuxley:1952} measured $20~\frac{m}{s}$ for the speed of the \gls{AP}. By assuming that the electrical field across the membrane is $10^7~\frac{V}{m}$, one can estimate the speed at the exit of the ion channels as $10^3~\frac{m}{s}$.
	See our other estimation in
...
By using the result derived in \textbf{\hyperlink{phys:CellPressure}{Physics panel~\ref{phys:CellPressure}}},
	...
	the change of the radius $\Delta r=10^{-9}\ [m]$ multiplied by the pressure $\Delta P$, that gives
\[	
...
\]
	...
	pressure is $10^6~\frac{N}{m^2}$, which is well above the measured value. This underpins our conclusion that the overwhelming part of the pressure (force) is of electrostatic origin.
	}
...
\physicspanel{phys:EnergyOfInformationTransfer}
{Estimating the energy cost of information transfer}
{
The energy dissipated during issuing an \gls{AP},
\textbf{\hyperlink{phys:DissipatedEnergy}{Physics panel~\ref{phys:DissipatedEnergy}}}

The theoretically lowest possible energetic cost of information is $k_B T ln(2))$, numerically ca. \textbf{\SI{3e-21}{J}}
i.e., compared to the optimal limit set by physics, this efﬁciency value is $10^6$-$10^8$ times less energy efﬁcient, in line with~\cite{EnergyNeuralCommunication:2021}. Nature protests:
