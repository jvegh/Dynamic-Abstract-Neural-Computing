...
The most important opening idea, electrically, is that they have a property called "charge" which is the same size, but opposite in polarity for the ion and the electron and the ions can have multiple charge.
...
Further complication arise from bioelectricity and neurotransmitters. They, apparently, "produce" new charges in a biological matter. 
...
\index{Kirchoff's Laws}
\textit{The charge, instead of current, cannot disappear (and cannot newly appear either), it has to go somewhere}. \textbf{\textit{There is no conservation law for the time derivative of the charge, only for the charge itself.
...
\subsubsection[Conduction]{Conduction mechanism\label{sec:PHYSICS_ConductionMechanism}}
...
is interpreted as the interaction of charge carriers with the particles in the material that are transported by a driving force.
In solid state, the volume considered is "infinitely large" (that is, whe field is homogeneous
and the propagation is isotropic), and the charge carriers are uniform that is only one type of interaction is present.
In biological objects, the volume is more or less limited (is finite), there may be charged objects present that make the propagation non-isotropic and the charge carriers are non-uniform
that introduces termodynamic interaction in addition to the electrical interaction.
...
The ions suffer (essentially elastic) collision with neutral molecules and transfer their momentum (and so: energy) to them. 
From the point of ions, the process is energy loss and momentum loss.
From the point of neutral molecules, the process is energy gain and momentum gain. If the acceleration occurs in a narrow tube 
...
Furthermore, the momentum loss results in momentum gain of the neutral molecules. Strangely enough, this way the electrical force
...
is limited to one direction, both the "lost" momentum of the ions and the "gained" momentum of the neutral particles.
...
Those accelerated molecules having the same velocity
may represent a significant "impact force", sufficient
to cause significant mechanical changes throughout
biological systems.
\index{impact force}
Notice that the momemntum transfer by collisions
is a two-way process. If a small amount of ionized
...
Vice versa, if the bulk of the particles are moved mechanically (say, when an ultrasound wave
 presses the neuron), the one-directionally moving
 neutral molecules transfer momentum to the charged particles,
 this way producing measurable electrical waves (such as \gls{AP}).
...
we can define a current for charges moving in an external force field,
...
The charge propagation mechanisms for electron and ions are entirely different (see also Fig.~\ref{tab:Electric_ResistanceTypes}).
...
by Stokes's Law). There are crucial differences between the current propagation's mechanisms in solids and in biological materials. 
...
those secondary quantities, we may use non-macthing quantity pairs (measured at different times), that produces wrong (unphysical) conclusions.
