...
\subsection[Measuring issues]{Measuring issues\label{sec:PHYSICS_MEASURING}}
\warningbox{
An intuitive grasp of concepts of measuring, electricity and electrical circuits, and thermodynamics, is helpful for understanding some of the basic theory concpts in cellular neurophysiology.}


\subsubsection{Electrical measurement\label{sec:Physics-MeasurementElectrical}}
The difficulties of performing \hypertarget{electric_conductance}{electric measurement} on living matter
...
“from Hyperphysics by Rod Nave, Georgia State University". \copyright \href{hyperphysics.phy-astr.gsu.edu/hbase/}{hyperphysics.phy-astr.gsu.edu/hbase/}
...
However, \textit{to measure conductance, we must generate charge inside the measured system} (see Fig.~\ref{fig:Physics-Multimeter}): we must apply some
...
changes during the measurement (see section~\ref{sec:Calculating-ion's-speed}), or a "foreign" (not considered)
...
values for the same membrane.\index{resting conductance}
...
voltage.
(Assuming a resting conduction in
axons is a self-contradiction. To have conduction, \textit{free charge carriers}
need to be present, which means the presence of ions inside the axon, that means potential
...
\hypertarget{Biologicalcircuits}{Biological electrical circuits} are different: their voltage continuously changes 
due to material transport and the currents are a million times slower. Biological interactions are governed
...
Ohm's Law is valid only in its \textit{differential form} but only in special cases in its \textit{integral form}. 
\index{Ohm's Law} 
...
The two definitions are equivalent only if the current's speed is infinitely large (instant interaction) or in other words, it does not depend on the time; furthermore, if the driving force is constant and purely of electrical origin.
...
good \textit{approximation} (but correct books also mention that on a 
\SI{300}{\milli\meter} %$30~cm$
piece of wire there exist a \SI{1}{\nano\second} delay).
However, the case is different in physiology: the speed of current is in the range of $m/s$; there is a "phase delay"
between the voltage and the current. 


\subsubsection[Capacitance]{Capacitance\label{sec:PHYSICS_MEASURINGSAPACITANCE}}
...
the condenser as one can observe that the voltage on that element 
...
\subsubsection[Conductance]{Interpreting conductance in electrolytes\label{fig:Physics-ElectricityOhmic}\label{sec:PHYSICS_MEASURINGCONDUCTANCE}}
...
At the positions labeled as "Electrotonic potentials" the neuron is in steady state: neither its condenser charges/discharges, nor its ion channels open.
...
Figure~\ref{fig:Physics-Clamping2} enables one to explain what is misinterpreted in biology in connection with Ohm's law (and why using a conductance meter in such a case is wrong).
\index{Ohm's law}
...
If one knows the laws of electricity, fits a saturating exponential to the curve as the membrane potential rises when the condenser charges and the physics is all right.
...
Non-understanding physics results in claiming non-Ohmic behavior.
...
The measurement must be fixed:  the 
...
notion; see its definition in section~\ref{sec:PHYSICS_MEASURINGCONDUCTANCE} and in section A.3.12 of~\cite{JohnstonWuNeurophysiology:1995}:
...
\index{ordinary laws}
Physics describes biological operations perfectly; although, it may use 'non-ordinary' laws (in this case: for slow current). Electric
...
adding some feedback current by \hyperlink{voltage-clamping}{clamping/patching}) when studying the electrical features of that system.
 ...
 \paragraph{Electrical and chemical invasions\label{sec:Physics-Invasions}}
...
See also section~\ref{sec:Fallacies-ElectricalCircuit}.

\subsubsection[Clamping/patching]{Voltage/current clamping/patching\label{sec:PHYSICS_clamping}}
...
(Fig.~7.2 of~\cite{PrinciplesNeuralScience:2013}) 
}
...
The figure clearly shows that the source of the "K conductance" and "K current"
...
\subsubsection{Interfacing biology}{\label{Physics-Interfacing}}
