...
Although science knows from the beginnings of scientific thinking that the material world around us
...
which connected the discrete and continuous views. Similarly, a new discipline was born when Planck had to hypothesize that energy also has
...
Thermodynamics could provide a framework for handling ions and determining their thermodynamical properties. However,
...
its validity for ionic solutions~\cite{VeghNon-ordinaryLaws:2025}. Classical science has limitations in describing nature;
especially in describing living nature.
...
%We must recall that 
Thermodynamics is about non-interacting particles (except their direct collisions as hard-ball points). When speaking about ions, the volume elements in the phase space are not independent: the energy and momentum of a particle depend on the other particles.
...
\gls{EM}
interaction, see section~\ref{sec:Fick-Electrodiffusion}.
...
paces, presenting a fascinating departure from traditional ('ordinary') physics.
\index{ordinary laws}
Another speed-related difference is that we must introduce
...
instead of a single value or uniform values of parameters. 
...
For a more careful experimenter, it is evident that this homogeneity
...
There are quasi-closed volumes in biological matter and the processes happen at the boundaries of the microscopic and macroscopic worlds, furthermore, we
...
course of the studied process. The inappropriate handling of mixing interaction speeds led to 'non-ordinary' behavior and one could conclude 'non-ordinary' laws when uses the appropriate approximation(s).
\index{non-ordinary}
...
The distributions of particles that comprise macroscopic quantities, however, can be calculated for charge-less or size-less 'material points' only.
The interference of those forces and that they affect two different 
features of the atomic particles led to unusual disciplinary consequences.
...
\end{equation}
\noindent
The formula considers an external force (essential: not exerted by another particle), the diffusion, the collisions that can change the particles' coordinates in the phase space, and the constraint force that originate from the medium and the container. Since that change must be the same for all particles,
...
and within a small fraction of nanometer distances. 
...
based on abstractions, which enable and require approximations and omissions.
...
From a physical point of view, \hypertarget{PhysicsLawsOfMotion}{ionic solutions} are confined to a well-defined volume, with a limited interaction with the rest of the world.
...
\physicspanel{phys:PlanckTimeDerivatives}{Calculating time derivatives from space derivatives}
{
...
\physicspanel{phys:PlanckStokesEinstein}{Considering the speed in electrolytes}
{In \textbf{Physics panel \ref{phys:PlanckTimeDerivatives}}
...
of an electrolyte can be described by this law of motion.
\index{laws of motion}
 For practical calculations, the voltage time derivative can be calculated directly from the input current, thereby accounting for the current production mechanism, see equations (\ref{eq:I_Out}) and~(\ref{eq:PSPderivative}), which directly consider the current production mechanism.
...
generates 
\textit{The electrical and thermodynamic phenomena are just two sides of the same coin} as discussed in~\cite{VeghUnifiedCrossdisciplinaryModel:2026},
and underpinned by
...
\noindent \hypertarget{NernstLaw}{known as \textit{Nernst equation}}.  In other words, \textit{Eq.(\ref{eq:NernstPlanck}) and Eq.(\ref{eq:Nernst1}) are the differential and integral formulations of the same knowledge}. As a generalization, by integration Eq.(\ref{eq:NernstPlanck}) one can derive the equivalent
thermodynamic voltage between any two points in the electrolyte. The limits of the integration are chosen arbitrarily (by choosing the reference concentration).
In a narrow tube embedded in membrane's wall (such as an ion channel), given that the membrane surfaces  at the same time
are condenser plates, the electrical field is constant, so in equilibrium state the concentration
changes logarthmically.
...
The Nernst potential (or reversal potential) is the specific membrane voltage where the net flow of a particular ion across a cell membrane stops, balancing the chemical concentration gradient with the electrical gradient, described by the Nernst equation.
 It's calculated using the ion's concentration ratio, its charge (valence), and temperature, indicating the potential at which an ion is in electrochemical equilibrium.
...
whether concentrations, or mobilities, or Nernst voltages, as it happens in the \gls{GHK} equation, see section~\ref{sec:Physics-Goldman-Hodgkin-Katz} and~\cite{VeghUnifiedCrossdisciplinaryModel:2026}.


\subsection[Fick's Law]{Fick's Lawfor electrodiffusion\label{sec:Fick-Electrodiffusion}}
\index{Fick's Law}

\physicspanel{phys:FicksLawElectrodiffusion}{Deriving Fick's Law for electrodiffusion}
{
...
The other way round, when we assume that the propagation of the electric field in the solution is $2*10^{8}\ m/s$ and multiply it by the factor above, we arrive at ion's speed in the range around $10^{2}\ m/s$.
