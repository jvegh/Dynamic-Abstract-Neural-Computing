...
\section[Modeling]{Modeling\label{sec:SingleModeli}}
...
\subsection[Modeling and disciplinarity]{Modeling and disciplinarity\label{sec:SINGLE_MultiPhysicsNeuronN}}
...
in this chapter the discussion remains on the surface, so
(we hope) the essence can be understood with knowing only
the elements of science and physiology. The deeper layers are explained
in the next chapter. However, the open mind is required
...
on the distance from the membrane. The lipid layer is partially permeable (controlled or uncontrolled) for the components above (channels, mainly ion channels).
...
If we consider that the ion flow means charge propagation in a membrane tube where the specific capacity depends on the thickness (of the myelin sheath) of the wall \cite{VeghMembranePotential:2026}, the faster propagation is not mysterious anymore.
