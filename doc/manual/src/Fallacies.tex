...
\chapter{Fallacies\label{ch:Fallacies}}
...
On the right side, in the extracellular segment, from the point also at a height of 0, we arrive at a point at a height of \SI{21}{\milli\volt} along the blue arrows in the same way. The potential difference between the two mentioned points is \SI{42.5}{\milli\volt}, which \gls{HH} measured (see section~\ref{sec:Single-UnifiedCurrents}). It is a correct value,
...
If, for any reason, the membrane voltage suddenly rises above the threshold value, due to an extremely fast $Na^+$ influx,
...
\begin{figure}
\includegraphics[width=.8\columnwidth]{fig/RestingPotential5.pdf}
...
"Clamping" (see section~\ref{sec:PHYSICS_clamping}) fixes the membrane voltage, meaning that the neuron cannot restore its initial
...
This voltage is given by the resultant of the green curved $Na^+[50]$ and the black dashed straight
...
This is where the urban legend "$K^+$ flows out through the membrane in the second phase of \gls{AP}, with a significant delay compared to the $Na^+$ influx" comes from.
It is really a good measurement, it is really $K^+$ ion, but \gls{HH} and their late descendants did not measure what they thought.
So there is no question of specific $Na^+$ and $K^+$ (synchronized with each other) channels: in both cases, the concentration and potential relationships decide what ions flow and in which direction.
...
\caption{Artificial and natural decay of the AP state.
...
it would disappear at zero clamping voltage.
In other words, the extra voltage and the  $K^+$ current disappear in the absence of clamping, as observed.
%
...
\section{Equivalent electrical circuit\label{sec:Fallacies-ElectricalCircuit}}
...
\section{Membrane as a wrong isolator\label{sec:Fallacies-Wrong isolator}}
